...
void draw_universe_backdrop(const UniversePalette &pal, std::uint64_t sig, Rectangle rect,
                            float t) {
    ...
    for (int i = 0; i < stars; ++i) {
        ...
        const float twinkle =
            0.35f + 0.65f * static_cast<float>(0.5 + 0.5 * std::sin(t * speed + phase));
        const float sz = bright ? 1.6f + 1.8f * static_cast<float>(fr())
                                : 0.6f + 0.9f * static_cast<float>(fr());
        Color sc = palette_color(pal.star);
        sc.a = static_cast<unsigned char>(
            std::clamp(twinkle * (bright ? 235.0f : 150.0f), 0.0f, 255.0f));
        if (bright) {
            Color halo = sc;
            halo.a = static_cast<unsigned char>(sc.a * 0.4f);
            DrawCircleGradient(static_cast<int>(sx), static_cast<int>(sy), sz * 3.2f, halo,
                               {0, 0, 0, 0});
        }
...
void draw_browser_modal(AppState &app, CosmosState &cosmos, Rectangle viewport, float scale) {
    ...
    draw_text("SAVED SANDBOXES", {modal.x + 18.0f * scale, modal.y + 16.0f * scale}, 17.0f * scale,
              WL::TEXT_PRIMARY);

    // Close button.
    const Rectangle close = {modal.x + modal.width - 38.0f * scale, modal.y + 14.0f * scale,
                             24.0f * scale, 24.0f * scale};
    if (draw_button(close, "x", {30, 18, 40, 235}, {60, 30, 70, 255}, WL::TEXT_PRIMARY, true,
                    scale) ||
    ...
    if (marks.empty()) {
        draw_text_block("No saved sandboxes yet. Spawn a scale and press Save to bookmark it - it "
                        ...
                        {modal.x + 18.0f * scale, modal.y + 56.0f * scale,
                         modal.width - 36.0f * scale, 80.0f * scale},
    ...
    for (int i = 0; i < shown; ++i) {
        ...
        draw_text(b.title.empty() ? b.seed : b.title, {row.x + 12.0f * scale, row.y + 7.0f * scale},
                  15.0f * scale, WL::TEXT_PRIMARY);
...
void draw_ladder(CosmosState &cosmos, Rectangle rect, float scale) {
    ...
    draw_text("SCALE LADDER", {rect.x + 14.0f * scale, rect.y + 12.0f * scale}, 13.0f * scale,
    ...
    for (std::size_t i = 0; i < kScaleCount; ++i) {
        ...
        const Rectangle row = {rect.x + 8.0f * scale, top + row_h * i, rect.width - 16.0f * scale,
                               row_h - 4.0f * scale};
        const bool active = (cosmos.scale == tier.scale);
        const bool hot = CheckCollisionPointRec(GetMousePosition(), row);
        const Color fill =
            active ? Color{12, 30, 50, 240} : (hot ? Color{9, 20, 36, 220} : Color{6, 13, 24, 180});
        ...
        draw_text(fmt_sci(tier.length_m) + " m", {row.x + 12.0f * scale, row.y + 24.0f * scale},
                  11.5f * scale, with_alpha(WL::TEXT_TERTIARY, 220));
        if (clicked(row)) {
...
void draw_inspector(const CosmosState &cosmos, Rectangle rect, float scale) {
    ...
    draw_card(rect, {6, 13, 24, 224}, with_alpha(WL::GLASS_BORDER, 120));
    draw_text("OBJECT LIBRARY", {rect.x + 14.0f * scale, rect.y + 12.0f * scale}, 13.0f * scale,
              with_alpha(WL::CYAN_CORE, 200));
    ...
    draw_text_block(
        o.description,
        {rect.x + 14.0f * scale, insp_y + 28.0f * scale, rect.width - 28.0f * scale, 30.0f * scale},
    ...
    const bool quantum_scale = scale_index(cosmos.scale) <= scale_index(Scale::NANOSCALE);
    if (quantum_scale) {
    ...
    if (quantum_scale) {
        ...
        draw_text("reduced C " + fmt_sci(quantum::reduced_compton_wavelength_m(o.rest_mass_kg)) +
                      " m   -   radius " + fmt_sci(quantum::in_planck_lengths(o.radius_m)) + " l_P",
    ...
    // Quantum Genesis — synthesize this universe's particle-physics viability
    // from its law genome. Shown on the subatomic tier (the first layer of
    // existence): can this physics even build stable matter?
    if (cosmos.scale == Scale::SUBATOMIC) {
        const genesis::QuantumUniverse qu = genesis::synthesize(cosmos.genome);
        draw_text("QUANTUM GENESIS", {rect.x + 14.0f * scale, y}, 11.5f * scale,
                  with_alpha(WL::XENON_CORE, 205));
        y += 16.0f * scale;
        draw_text_block(qu.verdict,
                        {rect.x + 14.0f * scale, y, rect.width - 28.0f * scale, 28.0f * scale},
                        11.5f * scale, WL::TEXT_SECONDARY, 2.0f * scale);
        y += 30.0f * scale;
        const std::string l1 = "n-p split " + fmt_fixed(qu.np_mass_diff_mev, 2) +
                               " MeV   deuteron " + (qu.deuteron_bound ? "bound" : "unbound") +
                               "   di-p " + (qu.diproton_bound ? "BOUND" : "unbound");
        const std::string l2 = "primordial He " +
                               std::to_string(static_cast<int>(qu.primordial_He * 100.0 + 0.5)) +
                               "%   max Z " + std::to_string(qu.max_stable_Z) + "   complexity " +
                               fmt_fixed(qu.complexity_score, 2);
    ...
    if (cosmos.compare_object >= 0 && cosmos.compare_object < static_cast<int>(all.size()) &&
        ...
        y += 18.0f * scale;
        const std::string line1 = "mass x" + fmt_fixed(cmp.mass_ratio, 2) + "  (" +
                                  (cmp.mass_orders >= 0 ? "+" : "") +
                                  std::to_string(cmp.mass_orders) + " orders)";
        const std::string line2 = "radius x" + fmt_fixed(cmp.radius_ratio, 2) + "   stability " +
                                  (cmp.stability_delta >= 0 ? "+" : "") +
...
void draw_observables(const CosmosState &cosmos, Rectangle rect, float scale) {
    draw_card(rect, {6, 13, 24, 224}, with_alpha(WL::GLASS_BORDER, 120));
    draw_text("LIVE OBSERVABLES", {rect.x + 14.0f * scale, rect.y + 10.0f * scale}, 13.0f * scale,
              with_alpha(WL::PLASMA_GREEN, 210));

    if (!cosmos.has_sim) {
        draw_text_block(
            "Spawn this scale to populate a live sandbox and read its emergent observables.",
    ...
    const Rectangle bar = {rect.x + 12.0f * scale, sig_y, rect.width - 24.0f * scale,
                           30.0f * scale};
...
void draw_dossier_modal(CosmosState &cosmos, Rectangle viewport, float scale) {
    ...
    draw_text(c.class_name, {m.x + 20.0f * scale, m.y + 34.0f * scale}, 24.0f * scale,
              WL::TEXT_PRIMARY);
    draw_text("seed '" + cosmos.seed + "'   -   generation v" + std::to_string(kGenerationVersion),
    ...
    auto tile = [&](int col, int row, const char *label, const std::string &value) {
        draw_metric({m.x + 20.0f * scale + col * (tw + 6.0f * scale),
                     gy + row * (th + 6.0f * scale), tw, th},
                    label, value, scale);
    };
    ...
    for (const std::string &tr : c.traits) {
        ...
        ty += 19.0f * scale;
        if (ty > m.y + m.height - 60.0f * scale)
            break;
    }
