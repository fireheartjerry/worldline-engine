...
inline const ParticleInfo *particle_table() {
    return detail::kParticleTable;
}
...
inline bool is_scalar_boson(Particle p) {
    return p == Particle::Higgs;
}
...
inline bool is_boson(Particle p) {
    return !is_fermion(p);
}
...
inline bool is_stable(Particle p) {
    return p == Particle::Up || p == Particle::Down || p == Particle::Electron || is_neutrino(p) ||
           p == Particle::Photon || p == Particle::Gluon;
}
...
inline double semf_pairing_mev(int Z, int A) {
    ...
    if (zEven && nEven)
        return +delta; // even-even
    if (!zEven && !nEven)
        return -delta; // odd-odd
    return 0.0;        // odd-A
}
...
inline constexpr std::size_t kSolarCount = sizeof(kSolarTable) / sizeof(kSolarTable[0]);
...
inline std::size_t solar_abundance_count() {
    return detail::kSolarCount;
}
