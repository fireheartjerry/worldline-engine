...
inline double joules_to_ev(double E_J) { return E_J / electron_volt_J; }
inline double ev_to_joules(double E_eV) { return E_eV * electron_volt_J; }
inline double joules_to_mev(double E_J) { return E_J / kMeV_J; }
inline double mev_to_joules(double E_mev) { return E_mev * kMeV_J; }

// ---------------------------------------------------------------------------
// Mass <-> energy (Einstein E = m c^2)
// ---------------------------------------------------------------------------

inline double rest_energy_J(double mass_kg) { return mass_kg * c2; }
inline double rest_energy_mev(double mass_kg) { return joules_to_mev(mass_kg * c2); }
inline double mass_from_energy_mev(double E_mev) { return mev_to_joules(E_mev) / c2; }
...
inline double compton_wavelength_m(double mass_kg) {
    if (mass_kg <= 0.0) return INFINITY;
    return h / (mass_kg * c);
}
...
inline double de_broglie_wavelength_m(double momentum) {
    if (momentum <= 0.0) return INFINITY;
    return h / momentum;
}
...
inline double de_broglie_wavelength_nr_m(double mass_kg, double velocity) {
    if (mass_kg <= 0.0 || velocity <= 0.0) return INFINITY;
...
inline double thermal_de_broglie_m(double mass_kg, double temperature_K) {
    if (mass_kg <= 0.0 || temperature_K <= 0.0) return INFINITY;
...
inline double decay_width_J_from_lifetime(double tau_s) {
    if (tau_s <= 0.0) return INFINITY; // tau -> 0  =>  infinitely broad
    return hbar / tau_s;
}
...
inline double lifetime_s_from_width_mev(double width_mev) {
    if (width_mev <= 0.0) return INFINITY; // stable
